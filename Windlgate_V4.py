...
if st.session_state.selected_lake: # This section checks if selected_lake has a value
    ...
    if weather_data is not None: # This checks if weather_data was successfully retrieved
        ...
        ax.plot(
            weather_data.index, weather_data["Temperature (°C)"], marker='o',
            label='Temperature (°C)', linestyle='--', linewidth=2
        )
        ax.plot(
            weather_data.index, weather_data["Wind Speed (m/s)"], marker='s',
            label='Wind Speed (m/s)', linestyle='-', linewidth=2
        )

        # Customization
        ax.set_title('Temperature and Wind Speed (3-Hour Intervals)', fontsize=16)
        ...
        st.subheader("Precipitation and Solar Irradiation Across the Day")

        # Plot Precipitation
        fig1, ax1 = plt.subplots(figsize=(12, 6))
        ax1.plot(
            weather_data2["Time"], weather_data2["Precipitation (mm)"], marker='o',
            label='Precipitation (mm)', linestyle='--', linewidth=2, color='blue'
        )
        ax1.set_title('Precipitation Across the Day', fontsize=16)
        ...
        ax2.plot(
            weather_data2["Time"], weather_data2["Solar irradiation (watt)"], marker='s',
            label='Solar irradiation (watt)', linestyle='-', linewidth=2, color='orange'
        )
        ax2.set_title('Solar Irradiation Across the Day', fontsize=16)
