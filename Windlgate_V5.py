...
if st.session_state.selected_lake: # This section checks if selected_lake has a value
    ...
    if weather_data is not None: # This checks if weather_data was successfully retrieved
        ...
        ax.plot(
            ...
            label='Temperature (°C)', linestyle='-', linewidth=2
        )
        ax.plot(
        ...
        # Next up we calculate dynamic limits for y-axis
    ...
    # Here we calculate the mean precipitation and solar irradiation
    ...
    # here we calculate the  mean pressure for the next 3 hours
    ...
    # here we calculate the mean humidity for the next 3 hours
    ...
    # here we make the wave height prediction
    ...
    if selected_lake["name"] == "Lake Silvaplanersee":
        ...
        # we default the title and embed an iframe for other lakes
    ...
    # Here we generate and display directions links (for this part of the code we used ChatGPT, for proper structuring)
...
# User Input and Map Display (if no lake is selected)
    ...
    #This is a list of lake dictionaries each containing the lake's name, coordinates and webcam URL (if available, if not we implemented a link that leads to the direct website of the webcam).
    swiss_lakes = [
