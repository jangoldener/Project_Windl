...
#If yes, a subheader "Temperature and Wind Speed (3-Hour Intervals)" is displayed to introduce the chart.
    if weather_data is not None:
        ...
        st.subheader("Precipitation and Solar Radiation (3-Hour Intervals)")
        
        # Ensure the Time column is in datetime format
        weather_data2["Time"] = pd.to_datetime(weather_data2["Time"])

        # Extract formatted time as "HH:MM"
        weather_data2["Formatted Hour"] = weather_data2["Time"].dt.strftime("%H:%M")

        # Set the formatted hour as the index
        weather_data2.set_index("Formatted Hour", inplace=True)

                # Display the area chart
        st.area_chart(weather_data2[["Precipitation (mm)", "Solar irradiation (watt)"]])
        ...
        st.subheader("Air Pressure (3-Hour Intervals)")
        ...
        st.subheader("Relative Humidity (3-Hour Intervals)")
        
        

        # Assuming weather_data4 is a pandas DataFrame
        # Extract every third value from the "Time" column for the x-tick labels
        x_ticks = weather_data4["Time"][::3]
        ...
        ax.set_ylim(0, 105)  # Assuming relative humidity is between 0% and 100%
    ...
    # Define the time range (next 3 hours)
    time_range_end = current_time + timedelta(hours=3)

    # Filter the DataFrame for the next 3 hours
    next_3_hours_data = weather_data[(weather_data.index > current_time) & (weather_data.index <= time_range_end)]

    # Calculate the mean temperature for the next 3 hours
    mean_temperature_next_3_hours = next_3_hours_data['Temperature (°C)'].mean()

    # Calculate the mean wind speed for the next 3 hours
    mean_wind_speed_next_3_hours = next_3_hours_data['Wind Speed (m/s)'].mean()
    ...
    # Filter the DataFrame for the next 3 hours based on the 'Time' column
    next_3_hours_data_weather2 = weather_data2[
        (weather_data2['Time'] > current_time) & (weather_data2['Time'] <= time_range_end)
    ]

    # Calculate the mean precipitation for the next 3 hours
    mean_precipitation_next_3_hours = next_3_hours_data_weather2['Precipitation (mm)'].mean()

    # Calculate the mean solar irradiation for the next 3 hours
    mean_solar_irradiation_next_3_hours = next_3_hours_data_weather2['Solar irradiation (watt)'].mean()

  


    # Ensure the index is a datetime object
    weather_data3.index = pd.to_datetime(weather_data3.index)
    ...
    # Filter the DataFrame for the next 3 hours
    next_3_hours_data_weather3 = weather_data3[(weather_data3.index > current_time) & (weather_data3.index <= time_range_end)]

    # Calculate the mean pressure for the next 3 hours
    mean_pressure_next_3_hours = next_3_hours_data_weather3['Luftdruck (hPa)'].mean()
    ...
    next_3_hours_data_weather4 = weather_data4[
        (weather_data4['Time'] > current_time) & (weather_data4['Time'] <= time_range_end)
    ]

    # Calculate the mean relative humidity for the next 3 hours
    mean_humidity_next_3_hours = next_3_hours_data_weather4['rel. Luftfeuchtigkeit (%)'].mean()
